...
<div class="space-y-6 pb-8 max-w-[1400px] mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-slate-500 text-xs mt-1 font-medium tracking-tight">Ubah data kategori barang yang terdaftar</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden p-6">
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" class="space-y-6">
            ...
            <!-- Icon -->
            <div x-data="{ selectedIcon: '{{ old('icon', $category->icon ?? 'tag') }}' }">
                ...
                <p class="text-[11px] text-slate-400 font-medium mb-3">Masukkan nama icon dari <a href="https://lucide.dev/icons" target="_blank" class="text-red-500 hover:underline">Lucide Icons</a>.</p>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-lg shadow-red-500/20 active:scale-95 flex items-center gap-2 cursor-pointer">
                    <i data-lucide="check" class="w-4 h-4"></i> Perbarui Kategori
                </button>
            </div>
        </form>
    </div>
